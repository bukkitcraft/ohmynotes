...
Searches run against title, body, and tag names with case-insensitive regular
expression matching. Tag filters narrow results by exact tag membership. The
search is deliberately simple — a linear scan over in-memory notes — which is
more than fast enough for a personal notes corpus and keeps the tool dependency
free.
"""
...
def search_notes(
    ...
    *,
    ...
    When a query is given, it must match within the title, body, or any tag.
    ...
    for note in notes:
        ...
        if query:
            ...
            if rx.search(note.body):
                score += 40
                fields.append("body")
            matched_tags = [t for t in note.tags if rx.search(t)]
            if matched_tags:
                score += 10
                fields.append("tag")
            if score == 0:
